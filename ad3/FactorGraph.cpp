...
int FactorGraph::RunMPLP(double lower_bound,
  ...
  bool multi_graph =  IsMultiFactorGraph();
  if (multi_graph) {
    if (verbosity_ > 1) {
      cout << "Factor graph contains only dense factors with multi-valued variables." << endl;
    }
    // EliminateVariableLogPotentialsMulti();
  }
  if (!multi_graph) {
    gammas0_.clear();
  ...
  for (t = 0; t < mplp_max_iterations_; ++t) {
    ...
    if (0 == (t % num_iterations_compute_dual)) {
      compute_dual = true;
    }
    ...
    if (compute_dual) {
      ...
      bool compute_primal = compute_dual;
      double primal_obj = 0.0;
      if (compute_primal) {
        for (int i = 0; i < variables_.size(); ++i) {
          primal_obj += variables_[i]->GetLogPotential() * (*posteriors)[i];
        }
        for (int i = 0; i < additional_log_potentials.size(); ++i) {
          primal_obj += (*additional_posteriors)[i] * additional_log_potentials[i];
        }
  ...
  *upper_bound = dual_obj;
  ...
  optimal = true; // TODO(atm): check optimality via weak agreement.
  if (optimal) {
    if (!fractional) {
      if (verbosity_ > 1) {
