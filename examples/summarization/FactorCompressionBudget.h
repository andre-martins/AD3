  ...
  void Maximize(const vector<double> &variable_log_potentials,
    ...
    for (int i = 0; i < length - 1; ++i) {
      ...
      for (int k = 0; k < num_states; ++k) {
        ...
        if (CountsForBudget(i+1, k)) {
          // The state that counts for the budget.
          // For this k, b = budget_ not allowed.
          if (num_bins == GetBudget()+1) --num_bins;
        ...
        for (int b = 0; b < num_bins; ++b) {
          ...
          for (int l = 0; l < GetNumStates(i); ++l) {
            if (CountsForBudget(i, l) && b == 0) continue;
    ...
    for (int b = 0; b < num_bins; ++b) {
      for (int l = 0; l < GetNumStates(length - 1); ++l) {
        if (CountsForBudget(length-1, l) && b == 0) continue;
